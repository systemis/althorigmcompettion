...
void process(vector<int> stnow, int scoreA, int scoreB, bool alex)
{
  ...
  int fisrt = stnow[0];
  int last = stnow[stnow.size() - 1];
  vector<int> ls = stnow;
  if (alex)
  {
    scoreA += fisrt;
    ls.erase(ls.begin());
    process(ls, scoreA, scoreB, false);

    scoreA -= fisrt;
    scoreA += last;
    ls.insert(ls.begin(), fisrt);
    ls.pop_back();
    process(ls, scoreA, scoreB, false);
    return;
  }
